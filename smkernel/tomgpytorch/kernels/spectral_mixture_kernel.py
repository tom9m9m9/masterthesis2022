...
class SpectralMixtureKernel(Kernel):
    ...
    Args:
        ...
        :attr:`ard_num_dims` (int, optional):
            ...
            It should be `d` if :attr:`x1` is a `n行 x d列` matrix. Default: `1`
            n->フレーム数　d->入力の次元数
        ...
        :attr:`active_dims` (tuple of ints, optional):
            ...
            corresponds to the indices（索引） of the dimensions. Default: `None`.
        :attr:`eps` (float):
    ...
    def _create_input_grid(self, x1, x2, diag=False, last_dim_is_batch=False, **params):
        ...
        Use this helper rather than manually creating a meshgrid.
    ...
    def return_m(self):
        return self.mixture_means
    def return_v(self):
        return self.mixture_scales[:,0,0]
    def return_w(self):
    ...
    def forward(self, x1, x2, last_dim_is_batch=False, **params):
        ...
        # Should make x1/x2 (b(バッチサイズ) x k(カーネル数) x n(フレーム数) x d(入力の次元)) for k mixtures
        #batchsizeは設定していないので x1=x1_
        x1_ = x1.unsqueeze(len(batch_shape))
        x2_ = x2.unsqueeze(len(batch_shape))

        #print('x1_={}'.format(x1_))
        #print('x1_size={}'.format(x1_.size()))
        ...
        #print('w={}'.format(mixture_weights))
